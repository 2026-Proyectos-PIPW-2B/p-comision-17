...
document
  ...
  ?.addEventListener("click", () => {
    ...
    // Validación estricta revisando que las strings no estén vacías y los números sean válidos
    if (
      titulo === "" ||
      autor === "" ||
      categoria === "" ||
    ...
    // Resetear el formulario de carga y cerrar el modal de Bootstrap
...
window.cargarDatosEnModalEditar = function (id) {
  ...
  if (producto) {
    ...
    document.getElementById("formatoEditar").value =
      producto.formato || "Tapa blanda";
...
document
  .querySelector("#modalEditar .btn-outline-primary")
  ?.addEventListener("click", () => {
    if (idProductoSeleccionado !== null) {
      let productos = obtenerProductos();
